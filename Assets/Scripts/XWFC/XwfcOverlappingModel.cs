...
namespace XWFC
{
    public class XwfcOverlappingModel : XwfcStm
    {
        ...
        private readonly VectorizorOther _vectorizor = new();
        ...
        private void EliminateIncompletePatternsOther()
        {
            Debug.Log("Eliminating patterns...");
            var timer = new Timer();
            timer.Start();
        ...
        private (Vector3Int minCoord, Vector3Int maxCoord) GetPatternExtents(int patternId)
        {
            ...
            var minCoord = new Vector3Int(int.MaxValue, int.MaxValue, int.MaxValue);
            var maxCoord = new Vector3Int(int.MinValue, int.MinValue, int.MinValue);
            for (int x = 0; x < _kernelSize.x; x++)
            {
                for (int y = 0; y < _kernelSize.y; y++)
                {
                    for (int z = 0; z < _kernelSize.z; z++)
                    {
                        ...
                        var tileOriginDifInPattern = patternCoord - atomCoord;
                        var tileMaxCoordDifInPattern = patternCoord + tile.Extent - atomCoord - new Vector3Int(1, 1, 1);

                        if (minCoord.x > tileOriginDifInPattern.x) minCoord.x = tileOriginDifInPattern.x;
                        if (minCoord.y > tileOriginDifInPattern.y) minCoord.y = tileOriginDifInPattern.y;
                        if (minCoord.z > tileOriginDifInPattern.z) minCoord.z = tileOriginDifInPattern.z;

                        if (maxCoord.x < tileMaxCoordDifInPattern.x) maxCoord.x = tileMaxCoordDifInPattern.x;
                        if (maxCoord.y < tileMaxCoordDifInPattern.y) maxCoord.y = tileMaxCoordDifInPattern.y;
                        if (maxCoord.z < tileMaxCoordDifInPattern.z) maxCoord.z = tileMaxCoordDifInPattern.z;
                    }
                }
            }
            ...
            for (int x = 0; x < _kernelSize.x; x++)
            {
                for (int y = 0; y < _kernelSize.y; y++)
                {
                    for (int z = 0; z < _kernelSize.z; z++)
                    {
                        ...
                        // If all coordinates of the tiles are already occupied, the tile 'fits' and will not be placed again; the pattern is still allowed. 
        ...
        private void SetCollapsedInGridPreProcess(Vector3Int coord, int chosenPatternId)
        {
            var wave = EmptyWave();
            if (chosenPatternId < 0) throw new NoMoreChoicesException("No more choices...");
            ...
            _patternWave.Set(coord, wave);
            _propagationQueue.Enqueue(coord);
            CollapseQueue.Insert(coord, AdjacencyMatrix.CalcEntropy(1));
        }
        ...
        private HashSet<(Vector3Int, float)> Propagate(Queue<Vector3Int> propQueue, ref PatternWave patternWave, IEnumerable<Vector3Int> offsets, ref Grid<int> atomGrid, PatternMatrix patternMatrix, bool ignoreConflict = false)
        {
            ...
            while (propQueue.Count > 0)
            {
                ...
                foreach (var offset in offsetArray)
                {
                    ...
                    {
                        var allowed = _vectorizor.GetAtIndex(i, allowedNeighbors) == 1;
        ...
        private List<ulong> UnionizeChoices(PatternMatrix patternMatrix, List<int> choiceInts, Vector3Int offset)
        {
            _watch.Start();
            ...
            for (var i = 1; i < choiceInts.Count; i++)
            {
                ...
                allowedNeighbors = _vectorizor.Or(allowedNeighbors, other);
            }
            _watch.Stop();
        ...
        private List<ulong> UnionizeChoicesAsync(PatternMatrix patternMatrix, List<int> choiceInts, Vector3Int offset, int firstIndex, int lastIndex)
        {
            if (lastIndex - firstIndex <= 6)
            {
                ...
                {
                    var other = patternMatrix.GetRowVectors(choiceInts[i], offset);
                    result = _vectorizor.Or(result, other);
                }
                return result;
            }
        ...
        protected override void ProcessAtomSeeds()
        {
            foreach (var (tileId, atomCoord, coord) in AtomSeeds)
            {
                ...
                SetCollapsedInGridPreProcess(coord, patternId);
            }
